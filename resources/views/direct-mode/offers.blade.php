...
<script>
        ...
        function offerStatus(app) {
            ...
            if (validTill(app) < new Date()) return 'expired';
            return 'active';
        }
        ...
        function deriveOffers() {
            ...
            state.pending = state.applications.filter(app => ['under_review', 'shortlisted', 'interview_scheduled'].includes(String(app.application_status || '').toLowerCase()));
        }
        ...
        function renderMetrics() {
            ...
            const items = [
                ...
                [state.pending.length, 'Pending Offers', state.pending.length ? 'In company process' : 'Awaiting response', 'clock', 'orange-soft'],
        ...
        function renderOtherOffers() {
            const items = [
                ...state.offers.filter((item, index) => index > 0 || item.status !== state.activeTab).map(item => ({ ...item, kind: 'offer' })),
                ...state.pending.map(app => ({ app, status: 'pending', kind: 'pending' })),
            ];
            ...
            wrap.innerHTML = items.map(({ app, status, kind }) => {
                const j = job(app);
                const label = kind === 'pending' ? applicationStatusLabel(app) : statusLabel(status);
                return `<div class="other-row" data-job-id="${j.id || ''}"><div class="logo navy">${logoHtml(app)}</div><div class="offer-info"><h3>${escapeHtml(j.title || 'Job Offer')}</h3><p>${escapeHtml(companyName(app))}</p><div class="meta"><span><span class="icon" data-icon="pin"></span>${escapeHtml(j.location || 'Location')}</span><span><span class="icon" data-icon="briefcase"></span>${escapeHtml((j.job_type || 'Full Time').replace('_', ' '))}</span><span>${escapeHtml(j.salary || 'Salary not disclosed')}</span></div></div><div class="status"><span class="pill ${kind === 'pending' ? 'orange' : statusTone(status)}">${escapeHtml(label)}</span><small>${escapeHtml(label)} on ${escapeHtml(formatDate(offerDate(app)))}</small></div><span class="icon" data-icon="chevron-right"></span></div>`;
            }).join('');
            hydrateIcons(wrap);
        ...
        function renderNextSteps() {
            const hasActive = state.offers.some(item => item.status === 'active');
            const hasAccepted = state.offers.some(item => item.status === 'accepted');
            const steps = hasAccepted
                ? [['Complete Onboarding', 'Submit required documents after acceptance.', 'green-soft'], ['Prepare Joining Documents', 'Keep resume, ID and education proofs ready.', 'blue-soft'], ['Track Activity', 'Watch company updates in activity panel.', 'orange-soft']]
                : hasActive
                    ? [['Review Offer Letter', 'Go through offer details, CTC, role and validity.', 'green-soft'], ['Accept the Offer', 'Accept active offer before the validity date.', 'blue-soft'], ['Complete Onboarding', 'Submit required documents after acceptance.', 'orange-soft']]
                    : state.pending.length
                        ? [['Track Company Updates', 'Your shortlisted/interview applications are still in process.', 'green-soft'], ['Prepare Documents', 'Keep resume and certificates ready.', 'blue-soft'], ['Apply More Jobs', 'Increase offer chances with matching direct jobs.', 'orange-soft']]
                        : [['Apply to Jobs', 'Browse direct jobs and submit applications.', 'green-soft'], ['Complete Profile', 'A complete profile improves selection chances.', 'blue-soft'], ['Check Activity', 'Application updates will appear in activity.', 'orange-soft']];
        ...
        function renderDocuments() {
            const docs = state.offers.length
                ? [['Offer Letter', 'Available after company shares document'], ['Compensation Details', 'Based on offered salary/CTC'], ['Company Policy', 'Provided by company during onboarding']]
                : [['Resume', 'Upload latest resume from profile'], ['Education Proof', 'Keep marksheets ready'], ['ID Proof', 'Required during onboarding']];
        ...
        function applicationStatusLabel(app) {
            return String(app.application_status || 'pending').replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase());
        }
