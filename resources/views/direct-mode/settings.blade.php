...
<script>
        ...
        function renderAccount() {
            ...
            $('[data-panel-root]').innerHTML = `
                <div class="profile-form">
                    <div class="photo"><label>Profile Photo</label><div class="photo-img">${p.profile_photo ? `<img src="${escapeAttr(p.profile_photo)}" alt="Profile photo">` : ''}</div><small>JPG, PNG or GIF. Max size 2MB.</small></div>
        ...
        function renderLinked() {
            $('[data-panel-root]').innerHTML = `
                <div class="toggle-row"><div><h3>Google Account</h3><p>${state.user.email || 'Not connected'}</p></div><button class="outline" type="button">Connected</button></div>
                <div class="toggle-row"><div><h3>LinkedIn</h3><p>Connect LinkedIn profile for better visibility</p></div><button class="outline" type="button">Connect</button></div>`;
        }
        ...
        async function loadData() {
            if (token) {
                ...
                state.profile = profileRes.data?.profile || profileRes.data?.fresher_profile || profileRes.data || {};
                state.dashboard = dashboardRes.data || {};
                state.user = state.dashboard.user || state.user;
            }
            updateUserChrome();
            render();
        }
