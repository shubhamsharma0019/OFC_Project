...
<script>
        ...
        function statusInfo(app) {
            if (hasCompletedInterview(app)) return ['Meeting Completed', 'orange', 3];
            return statusMeta[statusKey(app)] || [titleCase(statusKey(app).replaceAll('_', ' ')), 'blue', 1];
        }

        function hasCompletedInterview(app) {
            const interview = app.interview || {};
            if (!interview.id && !interview.interview_date) return false;
        ...
        async function loadApplications() {
            if (!token) {
                $('[data-app-list]').innerHTML = '<div class="empty">Please log in to view applications.</div>';
            ...
            try {
                ...
                if (!response.ok || payload.success === false) throw new Error(payload.message || 'Applications could not be loaded.');
