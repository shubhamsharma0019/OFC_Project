            ...
            <div class="space-y-8">
                ...
                <article id="latestTrainingCard" class="relative flex min-h-[220px] items-center overflow-hidden rounded-lg border border-[#dce7f8] bg-gradient-to-r from-white to-[#eef5ff] p-8 shadow-[0_10px_24px_rgba(6,25,66,.04)]">
                    ...
                    <div class="absolute bottom-7 right-12 hidden h-[155px] w-[155px] rotate-[-12deg] items-center justify-center rounded-full bg-[#e1edff] text-[#075fe4] sm:flex">
                        <svg class="h-20 w-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            <path d="M4 19V5"></path>
...
<script>
    ...
    function icon(name) {
        return `<span class="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[#f0f5ff] text-[#075fe4] [&>svg]:h-5 [&>svg]:w-5 [&>svg]:fill-none [&>svg]:stroke-current [&>svg]:stroke-2 [&>svg]:[stroke-linecap:round] [&>svg]:[stroke-linejoin:round]">${dashboardIcons[name] || dashboardIcons.course}</span>`;
    }
    ...
    function renderDashboard(data, unread) {
        ...
        dashboardStatsGrid.innerHTML = [
            ...
            smallCard(icon('training'), 'Current Training', stats.active_trainings || stats.total_course_enrollments || 0),
            smallCard(icon('notification'), 'Notifications', unread || 0),
        ].join('');

        quickActions.innerHTML = [
            actionCard(icon('profile'), 'Complete Profile', '/fast-track/profile', Number(profile.profile_completion || 0) >= 100),
            actionCard(icon('assessment'), 'Initial Assessment', '/fast-track/assessment', !!assessment),
            actionCard(icon('course'), 'Explore Fast Track', '/fast-track/courses', !!latestEnrollment),
            actionCard(icon('progress'), 'View Training', '/fast-track/training', Number(stats.total_course_enrollments || 0) > 0),
        ].join('');

        const activities = [];
        if (profile.profile_id) activities.push([icon('profile'), 'Profile loaded successfully', profile.qualification || profile.city || 'Profile active']);
        if (assessment) activities.push([icon('assessment'), 'Initial assessment completed', FastTrack.date(assessment.submitted_at)]);
        (data.recent_applications || []).forEach((item) => activities.push([icon('training'), 'Applied for ' + FastTrack.courseName(item.job || { course_name: item.job?.title || 'Job' }), FastTrack.date(item.applied_at)]));
        (data.recent_certificates || []).forEach((item) => activities.push([icon('certificate'), 'Certificate earned', FastTrack.date(item.created_at)]));
        if (latestEnrollment) activities.push([icon('enroll'), 'Enrolled in ' + FastTrack.courseName(latestEnrollment.course), FastTrack.date(latestEnrollment.enrollment_date)]);
    ...
    function renderProfileMissing() {
        dashboardStatsGrid.innerHTML = [
            ...
            smallCard(icon('training'), 'Current Training', 0),
            smallCard(icon('notification'), 'Notifications', 0),
        ].join('');
        quickActions.innerHTML = [
            actionCard(icon('profile'), 'Complete Profile', '/fast-track/profile', false),
            actionCard(icon('assessment'), 'Initial Assessment', '/fast-track/assessment', false),
            actionCard(icon('course'), 'Explore Fast Track', '/fast-track/courses', false),
            actionCard(icon('progress'), 'View Training', '/fast-track/training', false),
        ].join('');
